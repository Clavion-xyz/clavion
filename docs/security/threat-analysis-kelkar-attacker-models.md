# ISCL vs. "Rethinking Crypto Attacker Models" (Kelkar, Cornell/a16z)

Analysis of ISCL's resilience against attack vectors from Mahimna Kelkar's a16z crypto research seminar on novel attacker models, TEE-based encumbrance attacks, and smart contract collusion.

**Source:** a16z Crypto Research Seminar — "Rethinking Crypto Attacker Models" by Mahimna Kelkar (PhD, Cornell, advised by Ari Juels)

---

## 1. Encumbrance Attacks (TEE-based key splitting)
...
|---|---|
| One key = one owner | ISCL doesn't care who "owns" the key — it just enforces policy before signing |
| User has full key access | ISCL *requires* the user to have full access (passphrase to unlock) |
| User won't split key functionality | Not relevant — ISCL is the user's own tool, not a multi-party protocol |

**Where it *could* matter:** If someone deployed ISCL inside a TEE and sold partial signing access (e.g., "sign any transfer < 0.1 ETH"), they'd be using ISCL as a tool *within* an encumbrance scheme. But this isn't an attack *on* ISCL — it's an attack on whatever protocol trusts the resulting signatures. ISCL's PolicyEngine would still enforce its rules (allowlists, value caps, approval requirements) regardless of who triggers the signing.

**Bottom line:** ISCL's security doesn't depend on the "single entity ownership" assumption that encumbrance attacks violate. Our trust model is: *user is trusted, AI agent is not*.

---

## 2. Smart Contract-Based Collusion ("Smart Collusion")
...
ISCL's architecture is explicitly *centralized* at the user level — one user, one keystore, one policy engine. The "collusion" threat in ISCL is between a malicious skill (Domain A) and a compromised component (e.g., RPC endpoint), which we handle with defense-in-depth, not game-theoretic incentives.
...
This is actually ISCL's *core design scenario* — an AI agent trying to perform crypto operations. Our defenses:
...
## 4. "Soft Assumptions" in Security Models
...
| **ISCL Core itself isn't tampered with** | Low (trusted by definition) | v0.2: TEE/enclave deployment, code signing |
...
## 5. Proofs of Complete Knowledge
...
**ISCL doesn't need this** — we're not verifying that the user "really" owns their key in a protocol-theoretic sense. We just need the passphrase to decrypt the keystore. If the user wants to put their ISCL keystore passphrase in a TEE-based automated system, that's their prerogative — ISCL's policy engine still enforces all rules.
...
|---|---|---|---|
| TEE key encumbrance | No | N/A | ISCL doesn't assume single-entity ownership |
...
1. **"Soft assumptions"** — our v0.1 reliance on OS security and user attentiveness are real gaps, acknowledged in the threat model with v0.2 mitigations planned (hardware wallet, session keys, remote signer)
2. **AI as attacker** — this is literally our design scenario, and we handle it with the three-domain architecture + mandatory human approval
