...
This guide covers performance optimization for ISCL Core deployments, including SQLite tuning, RPC management, and throughput considerations.
...
**Key insight:** RPC calls account for 90%+ of pipeline latency. Optimizing SQLite or in-process code yields marginal gains. Optimizing RPC configuration has the largest impact.
...
WAL provides the best performance for ISCL's access pattern (many small writes, concurrent reads for the web dashboard). No change needed.
...
**Trade-off:** In `NORMAL` mode, a power failure (not a process crash) could lose the most recent transactions. Process crashes are still safe. For most ISCL deployments (single-user, local runtime), `NORMAL` is acceptable.
...
**Warning:** VACUUM rewrites the entire database and temporarily doubles disk usage. Run during maintenance windows.
...
- [ADR-002: SQLite for Audit Trail](../architecture/adrs/002-sqlite-audit-trail.md) -- SQLite design rationale
