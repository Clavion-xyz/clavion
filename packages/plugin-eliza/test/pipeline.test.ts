...
describe("executeSecurePipeline", () => {
  test("approve → sign → success", async () => {
    const client = {
      txApproveRequest: vi.fn().mockResolvedValue(makeApproveResponse(true, "tok-1")),
    ...
    expect(result.description).toBe("Transfer 1 USDC");
    expect(result.riskScore).toBe(15);
    expect(client.txApproveRequest).toHaveBeenCalledOnce();
  ...
  test("user declines → returns declined result", async () => {
    const client = {
  ...
  test("returns risk score and reasons on decline", async () => {
    const response = makeApproveResponse(false);
    response.riskScore = 75;
    response.riskReasons = ["high_value_transfer", "unknown_contract"];
    ...
    expect(result.riskReasons).toEqual(["high_value_transfer", "unknown_contract"]);
  });
  ...
  test("handles broadcast failure", async () => {
    const signResponse = {
      ...makeSignResponse(),
      broadcast: false,
      broadcastError: "rpc_unavailable",
    };
    ...
    expect(result.broadcastError).toBe("rpc_unavailable");
  });

  test("includes intentId in result", async () => {
    const client = {
      txApproveRequest: vi.fn().mockResolvedValue(makeApproveResponse(true, "tok-1")),
      txSignAndSend: vi.fn().mockResolvedValue(makeSignResponse()),
    } as unknown as ISCLClient;
