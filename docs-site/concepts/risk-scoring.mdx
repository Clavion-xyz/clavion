---
title: Risk Scoring
description: How Clavion quantifies transaction risk on a 0-100 scale
icon: gauge
---
...
<Steps>
  ...
  <Step title="Risk context assembly">
...
<Accordion title="Example 1: Simple allowlisted ETH transfer -- Score 0">
...
<Accordion title="Example 2: Swap with unknown token, high slippage -- Score 35">
...
<Accordion title="Example 3: Unlimited approval to unknown contract -- Score 75">
...
<Accordion title="Example 4: Reverted simulation with unknown contract -- Score 90">
...
<Tabs>
  <Tab title="Conservative (DeFi Treasury)">
  ...
  <Tab title="Permissive (Development)">
    Set `maxRiskScore: 100`, leave allowlists empty, set value limits to `"0"`. Only simulation reverts will produce nonzero scores, and even those will not trigger approval requirements.
  ...
  <Tab title="Balanced (Production Agent)">
    Use the defaults (`maxRiskScore: 50`, empty allowlists). Transactions interacting with unknown contracts (+40) or experiencing simulation reverts (+50) will require approval. Routine allowlisted transfers pass through automatically.

    ```json
    {
      "version": "1",
      "maxValueWei": "10000000000000000000",
      "maxApprovalAmount": "10000000000000000000",
      "contractAllowlist": [],
      "tokenAllowlist": [],
      "allowedChains": [1, 10, 42161, 8453],
      "recipientAllowlist": [],
      "maxRiskScore": 50,
      "requireApprovalAbove": { "valueWei": "1000000000000000000" },
      "maxTxPerHour": 20
    }
    ```
  </Tab>
...
- [Policy Engine](/concepts/policy-engine) -- How policy rules enforce security decisions
- [Configuration Reference](/reference/config-reference) -- Full `PolicyConfig` schema and loading behavior
- [Trust Domains](/concepts/trust-domains) -- How Domain B enforces the policy boundary
