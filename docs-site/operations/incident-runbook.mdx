...
description: Symptom-indexed guide for diagnosing operational issues
...
<Steps>
  <Step title="Find the symptom">
    Find the symptom below that matches what you are observing.
  </Step>
  <Step title="Follow the diagnosis">
    Follow the diagnosis steps in order.
  </Step>
  <Step title="Apply the resolution">
    Apply the recommended resolution.
  </Step>
  <Step title="Verify via audit trail">
    Check the [Audit Trail](/operations/audit-trail) for root cause analysis.
  </Step>
</Steps>
...
<Accordion title="Unauthorized Transaction Executed" icon="circle-exclamation">
...
<Accordion title="All Transactions Being Denied" icon="ban">
...
Review and update the policy configuration. See [Configuration Reference](/reference/config-reference) for all PolicyConfig fields.

</Accordion>

<Accordion title="Transactions Failing on Broadcast" icon="tower-broadcast">
...
<Accordion title="RPC Errors / 502 Responses" icon="plug-circle-exclamation">
...
<Accordion title="Approval Requests Timing Out" icon="clock">
...
<Accordion title='Signing Fails "No key found"' icon="key">
...
<Accordion title="Sandbox Skill Times Out" icon="hourglass-end">
...
<Accordion title="Preflight Simulation Fails" icon="flask-vial">
...
<Steps>
  <Step title="Get the intent ID">
    Find the intent ID from error messages, API responses, or logs.
  </Step>
  <Step title="Pull the full audit trail">
    ```sql
    SELECT event, timestamp, data FROM audit_events
  ...
  <Step title="Identify where the pipeline failed">
    Which event is the last one? What does its `data` show?
  </Step>
  <Step title="Check surrounding events for context">
    ```sql
  ...
  <Step title="Correlate with application logs">
...
- [Audit Trail](/operations/audit-trail) -- Event types and SQL query patterns
- [Error Codes](/reference/error-codes) -- HTTP error shapes and recovery
...
- [Performance Tuning](/operations/performance-tuning) -- Resource optimization
