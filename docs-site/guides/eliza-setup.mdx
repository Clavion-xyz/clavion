---
title: ElizaOS plugin setup
description: Replace @elizaos/plugin-evm with Clavion's secure signing pipeline for ElizaOS agents.
icon: robot
---
...
<Steps>
  ...
  <Step title="Configure the Character File">
  ...
  <Step title="Start the ElizaOS Agent">
    ```bash
...
<Steps>
  ...
  <Step title="Remove the private key">
    Remove `EVM_PRIVATE_KEY` from `settings.secrets`.
  </Step>

  <Step title="Add the Clavion plugin">
    Add `"@clavion/plugin-eliza"` to the `plugins` array.
  </Step>

  <Step title="Add ISCL settings">
...
- [Eliza Adapter Plan](/docs/integrations/eliza-adapter-plan) -- Original design plan with code patterns
- [Adapter Development Tutorial](/guides/custom-adapter) -- How adapters are built
- [API Reference](/api-reference/overview) -- Endpoints used by the plugin
- [Configuration Reference](/configuration) -- Environment variables and policy settings
