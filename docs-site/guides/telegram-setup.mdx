...
description: Set up the Clavion Telegram bot for chat-based crypto operations with inline approval keyboards.
icon: paper-plane
---
...
4. Save this token -- you will need it for `TELEGRAM_BOT_TOKEN`
...
<Warning>
If `ISCL_TELEGRAM_ALLOWED_CHATS` is empty, the bot accepts commands from **any** chat. Always set this in production.
</Warning>

### Start ISCL Core with web approval mode
...
- [API Reference](/reference/api) -- Endpoints used by the bot
- [Web Approval UI](/guides/building-agents) -- Web-based approval dashboard
- [Multi-Chain Operations](/guides/multi-chain) -- Chain configuration
